# ============================================================
# SAM3/COCO -> POLÍGONOS ESPACIAIS
# Associação:
#   nome da imagem -> grids$image_id -> coordenadas EPSG:31982
#
# Requer:
#   sf
#   terra
...
ann_folds <- dir(
  "AI-DATA/ANN/AUTO-ANN/GSAT-BEL/SAM3/20260916T214028Z_556274fa/",
  full.names = TRUE
)

ann_folds <- ann_folds[!grepl("\\.", basename(ann_folds))]

all_ann <- paste0(
  ann_folds,
  "/full/annotations.coco.json"
)

existz = file.exists(all_ann)

all_ann <- all_ann[existz]
...
grids <- read_sf(
  "IN-PREP/Google_Satellite_Grades/grades_urbanas.gpkg"
)

grids
st_crs(grids)

stopifnot("image_id" %in% names(grids))

# image_id precisa identificar cada imagem/grade
if (anyDuplicated(grids$image_id)) {
  stop("Há image_id duplicado no arquivo de grades.")
}

# Índice rápido:
# nome do vetor = image_id
# valor          = linha do objeto em grids
grid_index <- seq_len(nrow(grids))
names(grid_index) <- as.character(grids$image_id)
...
# ============================================================
# 5. TESTE COM A POSIÇÃO 100
# ============================================================

posicoes <- 1:length(all_ann)

library(snow)
cl = makeCluster(3)
clusterExport(cl,list("convert_one_coco","all_ann","grids","grid_index","get_spatial_image_id","decode_coco_rle","mask_to_spatial_polygon"))
clusterEvalQ(cl,library(sf))

all_pols = pbapply::pblapply(cl=cl,posicoes,function(posicao){
  poligonos_100 <- convert_one_coco(
    json_file = all_ann[posicao],
    grids = grids,
    grid_index = grid_index,
    clip_to_grid = FALSE
  )
  return(poligonos_100)
})

saveRDS(object = all_pols,file = "C:/Users/Lucas Colares/Documents/polygons_gsat_bel_list.RData")
all_pols = readRDS("C:/Users/Lucas Colares/Documents/polygons_gsat_bel_list.RData")

stopCluster(cl)

pol_bind = do.call("rbind",all_pols)
nrow(pol_bind)
plot(pol_bind$geometry)

buf_trees = st_buffer(pol_bind,100)
plot(buf_trees$geometry)
buf_trees = st_union(buf_trees)

#install.packages("mapedit")
typez = unlist(lapply(lapply(all_pols,is),"[[",1))

pol_bind = mapedit:::combine_list_of_sf(all_pols[typez!="NULL"])

write_sf(pol_bind,"C:/Users/Lucas Colares/Documents/trees_gsat_bel.kml")

library(terra)
pol_list = readRDS("C:/Users/lucascolares/Documents/polygons_gsat_bel_list.RData")
typez = unlist(lapply(lapply(pol_list,is),"[[",1))
pol_list = pol_list[typez!="NULL"]

vect1 = vect(pol_list[[1]])

library(snow)
cl = makeCluster(8)
clusterExport(cl,list("pol_list"))
clusterEvalQ(cl,library(terra))
clusterEvalQ(cl,library(sf))

simplify_list = pbapply::pblapply(cl=cl, 1:length(pol_list),function(x){
  vect1_simple = simplifyGeom(vect(pol_list[[x]]), tolerance=0.5, preserveTopology=TRUE, makeValid=TRUE)
  return(vect1_simple)
})

plot(vect1_simple)

pol_bind = vect("C:/Users/lucascolares/Documents/trees_gsat_bel.kml")

all_union = lapply(pol_list,st_union)
uni_bind = mapedit:::combine_list_of_sf(all_union)
uni_bind2 = st_union(uni_bind)

pol_bufs = st_buffer(uni_bind2,10)
pol_bufs2 = st_buffer(uni_bind2,-10)
single_polygons <- st_cast(pol_bufs2, "POLYGON")
